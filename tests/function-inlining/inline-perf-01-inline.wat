(module
  ...
  (func $inc (param $var0 i32) (result i32)
    ;; Variables
  ...
  )                                                                          ;; END 'inc' function definition.
  
  (export "inc" (func $inc))
  
  (func $main (result i32)
    ;; Variables
    (local $var3 i32)                                                          ;; Variable: s
    (local $var4 i32)                                                          ;; Variable: i
    
    ;; Temp Variables
    
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var3)                                                          ;; Set var 's' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var4)                                                          ;; Set var 'i' from stack
    (block $exit1                                                              ;; Outer block for breaking while loop.
      (loop $loop1                                                             ;; Inner loop for continuing while.
        ;; WHILE Test condition...
        (local.get $var4)                                                          ;; Place var 'i' onto stack
        (i32.const 5000000)                                                        ;; Put a 5000000 on the stack
        (i32.lt_s)                                                                 ;; Stack2 < Stack1
        (i32.eqz)                                                                  ;; Invert the result of the test condition.
        (br_if $exit1)                                                             ;; If condition is false (0), exit the loop
        ;; WHILE Loop body...
        (local.get $var3)                                                          ;; Place var 's' onto stack
        (i32.const 1)                                                              ;; Put a 1 on the stack
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var3)                                                          ;; Set var 's' from stack
        (local.get $var4)                                                          ;; Place var 'i' onto stack
