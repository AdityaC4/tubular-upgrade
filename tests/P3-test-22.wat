(module
  ...
  (func $Fibonacci (param $var0 i32) (result i32)
    ;; Variables
    (local $var2 i32)                                                          ;; Variable: val1
    (local $var3 i32)                                                          ;; Variable: val2
    (local $var4 i32)                                                          ;; Variable: step
    (local $var5 i32)                                                          ;; Variable: sum
    ...
    (local.set $var2)                                                          ;; Set var 'val1' from stack
    (i32.const 1)                                                              ;; Put a 1 on the stack
    (local.set $var3)                                                          ;; Set var 'val2' from stack
    (i32.const 2)                                                              ;; Put a 2 on the stack
    (local.set $var4)                                                          ;; Set var 'step' from stack
    (block $exit1                                                              ;; Outer block for breaking while loop.
      (loop $loop1                                                             ;; Inner loop for continuing while.
        ;; WHILE Test condition...
        (local.get $var4)                                                          ;; Place var 'step' onto stack
        (local.get $var0)                                                          ;; Place var 'n' onto stack
        ...
        ;; WHILE Loop body...
        (local.get $var2)                                                          ;; Place var 'val1' onto stack
        (local.get $var3)                                                          ;; Place var 'val2' onto stack
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var5)                                                          ;; Set var 'sum' from stack
        (local.get $var3)                                                          ;; Place var 'val2' onto stack
        (local.set $var2)                                                          ;; Set var 'val1' from stack
        (local.get $var5)                                                          ;; Place var 'sum' onto stack
        (local.set $var3)                                                          ;; Set var 'val2' from stack
        (local.get $var2)                                                          ;; Place var 'val1' onto stack
        (local.get $var3)                                                          ;; Place var 'val2' onto stack
        (i32.add)                                                                  ;; Stack2 + Stack1
