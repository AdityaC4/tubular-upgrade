(module
  ...
  (data (i32.const 14) "TWENTY\00")
  (data (i32.const 21) "one\00")
  (global $free_mem (mut i32) (i32.const 25))
  ...
  (func $TwentyOne (result i32)
    ;; Variables
    (local $var1 i32)                                                          ;; Variable: twenty
    (local $var2 i32)                                                          ;; Variable: one
    
    ;; Temp Variables
    
    (i32.const 14)                                                             ;; Load address of string literal
    (local.set $var1)                                                          ;; Set var 'twenty' from stack
    (i32.const 21)                                                             ;; Load address of string literal
    (local.set $var2)                                                          ;; Set var 'one' from stack
    (local.get $var1)                                                          ;; Place var 'twenty' onto stack
    (local.get $var2)                                                          ;; Place var 'one' onto stack
    call $_strcat                                                              ;; Concatenate two strings
  )                                                                          ;; END 'TwentyOne' function definition.
  
  (export "TwentyOne" (func $TwentyOne))
