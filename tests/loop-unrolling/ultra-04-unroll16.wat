(module
  ...
  (func $MemoryAccess (result i32)
    ;; Variables
    (local $var1 i32)                                                          ;; Variable: base
    (local $var2 i32)                                                          ;; Variable: stride1
    (local $var3 i32)                                                          ;; Variable: stride2
    (local $var4 i32)                                                          ;; Variable: stride3
    (local $var5 i32)                                                          ;; Variable: stride4
    (local $var6 i32)                                                          ;; Variable: accumulator
    (local $var7 i32)                                                          ;; Variable: offset1
    (local $var8 i32)                                                          ;; Variable: offset2
    (local $var9 i32)                                                          ;; Variable: offset3
    (local $var10 i32)                                                         ;; Variable: offset4
    (local $var11 i32)                                                         ;; Variable: i
    
    ;; Temp Variables
    
    (i32.const 1000)                                                           ;; Put a 1000 on the stack
    (local.set $var1)                                                          ;; Set var 'base' from stack
    (i32.const 7)                                                              ;; Put a 7 on the stack
    (local.set $var2)                                                          ;; Set var 'stride1' from stack
    (i32.const 11)                                                             ;; Put a 11 on the stack
    (local.set $var3)                                                          ;; Set var 'stride2' from stack
    (i32.const 13)                                                             ;; Put a 13 on the stack
    (local.set $var4)                                                          ;; Set var 'stride3' from stack
    (i32.const 17)                                                             ;; Put a 17 on the stack
    (local.set $var5)                                                          ;; Set var 'stride4' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var6)                                                          ;; Set var 'accumulator' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var7)                                                          ;; Set var 'offset1' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var8)                                                          ;; Set var 'offset2' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var9)                                                          ;; Set var 'offset3' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var10)                                                         ;; Set var 'offset4' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    ...
    (block $exit1                                                              ;; Outer block for breaking while loop.
      (loop $loop1                                                             ;; Inner loop for continuing while.
        ...
        (i32.const 2000000)                                                        ;; Put a 2000000 on the stack
        (i32.const 15)                                                             ;; Put a 15 on the stack
        ...
        ;; WHILE Loop body...
        (local.get $var1)                                                          ;; Place var 'base' onto stack
        (local.get $var11)                                                         ;; Place var 'i' onto stack
        (local.get $var2)                                                          ;; Place var 'stride1' onto stack
        (i32.mul)                                                                  ;; Stack2 * Stack1
        (i32.const 997)                                                            ;; Put a 997 on the stack
        (i32.rem_s)                                                                ;; Stack2 % Stack1
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var7)                                                          ;; Set var 'offset1' from stack
        (local.get $var1)                                                          ;; Place var 'base' onto stack
        (local.get $var11)                                                         ;; Place var 'i' onto stack
        (local.get $var3)                                                          ;; Place var 'stride2' onto stack
        (i32.mul)                                                                  ;; Stack2 * Stack1
        (i32.const 991)                                                            ;; Put a 991 on the stack
        (i32.rem_s)                                                                ;; Stack2 % Stack1
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var8)                                                          ;; Set var 'offset2' from stack
        (local.get $var1)                                                          ;; Place var 'base' onto stack
        (local.get $var11)                                                         ;; Place var 'i' onto stack
        (local.get $var4)                                                          ;; Place var 'stride3' onto stack
        (i32.mul)                                                                  ;; Stack2 * Stack1
        (i32.const 983)                                                            ;; Put a 983 on the stack
        (i32.rem_s)                                                                ;; Stack2 % Stack1
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var9)                                                          ;; Set var 'offset3' from stack
        (local.get $var1)                                                          ;; Place var 'base' onto stack
        (local.get $var11)                                                         ;; Place var 'i' onto stack
        (local.get $var5)                                                          ;; Place var 'stride4' onto stack
        (i32.mul)                                                                  ;; Stack2 * Stack1
        (i32.const 977)                                                            ;; Put a 977 on the stack
        (i32.rem_s)                                                                ;; Stack2 % Stack1
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var10)                                                         ;; Set var 'offset4' from stack
        (local.get $var6)                                                          ;; Place var 'accumulator' onto stack
        (local.get $var7)                                                          ;; Place var 'offset1' onto stack
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var6)                                                          ;; Set var 'accumulator' from stack
        (local.get $var6)                                                          ;; Place var 'accumulator' onto stack
        (local.get $var8)                                                          ;; Place var 'offset2' onto stack
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var6)                                                          ;; Set var 'accumulator' from stack
        (local.get $var6)                                                          ;; Place var 'accumulator' onto stack
        (local.get $var9)                                                          ;; Place var 'offset3' onto stack
        (i32.sub)                                                                  ;; Stack2 - Stack1
        (local.set $var6)                                                          ;; Set var 'accumulator' from stack
        (local.get $var6)                                                          ;; Place var 'accumulator' onto stack
        (local.get $var10)                                                         ;; Place var 'offset4' onto stack
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var6)                                                          ;; Set var 'accumulator' from stack
        (local.get $var6)                                                          ;; Place var 'accumulator' onto stack
        (i32.const 100000)                                                         ;; Put a 100000 on the stack
        (i32.rem_s)                                                                ;; Stack2 % Stack1
        (local.set $var6)                                                          ;; Set var 'accumulator' from stack
        (local.get $var1)                                                          ;; Place var 'base' onto stack
        (local.get $var11)                                                         ;; Place var 'i' onto stack
        ...
        (local.get $var1)                                                          ;; Place var 'base' onto stack
        (local.get $var11)                                                         ;; Place var 'i' onto stack
        (i32.const 14)                                                             ;; Put a 14 on the stack
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.get $var2)                                                          ;; Place var 'stride1' onto stack
        (i32.mul)                                                                  ;; Stack2 * Stack1
        (i32.const 997)                                                            ;; Put a 997 on the stack
        ...
        (local.set $var6)                                                          ;; Set var 'accumulator' from stack
        (local.get $var11)                                                         ;; Place var 'i' onto stack
        (i32.const 16)                                                             ;; Put a 16 on the stack
        (i32.add)                                                                  ;; Stack2 + Stack1
        (local.set $var11)                                                         ;; Set var 'i' from stack
    ...
    (local.get $var6)                                                          ;; Place var 'accumulator' onto stack
  )                                                                          ;; END 'MemoryAccess' function definition.
  
  (export "MemoryAccess" (func $MemoryAccess))
