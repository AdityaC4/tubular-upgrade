(module
  ...
  (func $FindPrime (param $var0 i32) (result i32)
    ;; Variables
    (local $var2 i32)                                                          ;; Variable: is_prime
    (local $var3 i32)                                                          ;; Variable: test_factor
    (local $var4 i32)                                                          ;; Variable: test_cap
    ...
    (block $exit1                                                              ;; Outer block for breaking while loop.
      (loop $loop1                                                             ;; Inner loop for continuing while.
        ...
        ;; WHILE Loop body...
        (i32.const 1)                                                              ;; Put a 1 on the stack
        (local.set $var2)                                                          ;; Set var 'is_prime' from stack
        (i32.const 2)                                                              ;; Put a 2 on the stack
        (local.set $var3)                                                          ;; Set var 'test_factor' from stack
        ...
        (local.set $var4)                                                          ;; Set var 'test_cap' from stack
        (block $exit2                                                              ;; Outer block for breaking while loop.
          (loop $loop2                                                             ;; Inner loop for continuing while.
            ;; WHILE Test condition...
            (local.get $var3)                                                          ;; Place var 'test_factor' onto stack
            (local.get $var4)                                                          ;; Place var 'test_cap' onto stack
        ...
        ;; Test condition for if.
        (local.get $var2)                                                          ;; Place var 'is_prime' onto stack
        (if                                                                        ;; Execute code based on result of condition.
          (then                                                                      ;; 'then' block
