(module
  ...
  (global $free_mem (mut i32) (i32.const 15))
  ...
  (func $ReverseString (param $var0 i32) (result i32)
    ;; Variables
    (local $var2 i32)                                                          ;; Variable: length
    (local $var3 i32)                                                          ;; Variable: lastChar
    (local $var4 i32)                                                          ;; Variable: remaining
    (local $var5 i32)                                                          ;; Variable: i
    
    ;; Temp Variables
    (local $temp0 i32)
    (local $temp1 i32)
    
    (local.get $var0)                                                          ;; Place var 's' onto stack
    (call $_strlen)                                                            ;; Call _strlen for size()
    (local.set $var2)                                                          ;; Set var 'length' from stack
    ;; Test condition for if.
    (local.get $var2)                                                          ;; Place var 'length' onto stack
    ...
    (block $exit1                                                              ;; Outer block for breaking while loop.
      (loop $loop1                                                             ;; Inner loop for continuing while.
        ...
        ;; WHILE Loop body...
        (local.get $var4)                                                          ;; Place var 'remaining' onto stack
    ...
    (local.get $var3)                                                          ;; Place var 'lastChar' onto stack
    i32.store8                                                                 ;; Store the char at the address
    (local.get $temp1)                                                         ;; Return address of the string
    (local.get $var4)                                                          ;; Place var 'remaining' onto stack
    (call $ReverseString)                                                      ;; Call function ReverseString
    call $_strcat                                                              ;; Concatenate two strings
  )                                                                          ;; END 'ReverseString' function definition.
