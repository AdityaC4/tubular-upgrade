(module
  ...
  (data (i32.const 14) "ABCDEFGHIJKLMNOPQRSTUVWXYZ\00")
  (global $free_mem (mut i32) (i32.const 41))
  ...
  (func $ToLetter (param $var0 i32) (result i32)
    ;; Variables
    (local $var2 i32)                                                          ;; Variable: s
    ...
    (if (result i32)                                                           ;; Setup for || operator
      ...
      (else 
        (local.get $var0)                                                          ;; Place var 'x' onto stack
        (i32.const 26)                                                             ;; Put a 26 on the stack
    ...
    (if                                                                        ;; Execute code based on result of condition.
      (then                                                                      ;; 'then' block
        (i32.const 63)                                                             ;; Put a char \63 on the stack
        (return)                                                                   ;; Halt and return value.
      )                                                                          ;; End 'then'
    )                                                                          ;; End 'if'
    (i32.const 14)                                                             ;; Load address of string literal
    (local.set $var2)                                                          ;; Set var 's' from stack
    (local.get $var2)                                                          ;; Place var 's' onto stack
    (local.get $var0)                                                          ;; Place var 'x' onto stack
    (i32.add)                                                                  ;; Computer address: base + index
    (i32.load8_u)                                                              ;; Load byte at computed address
  )                                                                          ;; END 'ToLetter' function definition.
  
  (export "ToLetter" (func $ToLetter))
