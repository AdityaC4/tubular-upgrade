(module
  ...
  (data (i32.const 14) "AUGMENT\00")
  (global $free_mem (mut i32) (i32.const 22))
  ...
  (func $LetterG (result i32)
    ;; Variables
    (local $var1 i32)                                                          ;; Variable: s
    
    ;; Temp Variables
    
    (i32.const 14)                                                             ;; Load address of string literal
    (local.set $var1)                                                          ;; Set var 's' from stack
    (local.get $var1)                                                          ;; Place var 's' onto stack
    (i32.const 2)                                                              ;; Put a 2 on the stack
    (i32.add)                                                                  ;; Computer address: base + index
    (i32.load8_u)                                                              ;; Load byte at computed address
  )                                                                          ;; END 'LetterG' function definition.
  
  (export "LetterG" (func $LetterG))
