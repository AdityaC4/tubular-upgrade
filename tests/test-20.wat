(module
  ...
  (data (i32.const 14) "0\00")
  (data (i32.const 16) "0123456789\00")
  (data (i32.const 27) "\00")
  (global $free_mem (mut i32) (i32.const 28))
  ...
  (func $Int2String (param $var0 i32) (result i32)
    ;; Variables
    (local $var2 i32)                                                          ;; Variable: digits
    (local $var3 i32)                                                          ;; Variable: is_neg
    (local $var4 i32)                                                          ;; Variable: out
    ...
    (if                                                                        ;; Execute code based on result of condition.
      (then                                                                      ;; 'then' block
        (i32.const 14)                                                             ;; Load address of string literal
        (return)                                                                   ;; Halt and return value.
      )                                                                          ;; End 'then'
    )                                                                          ;; End 'if'
    (i32.const 16)                                                             ;; Load address of string literal
    (local.set $var2)                                                          ;; Set var 'digits' from stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    (local.set $var3)                                                          ;; Set var 'is_neg' from stack
    ;; Test condition for if.
    (local.get $var0)                                                          ;; Place var 'val' onto stack
    (i32.const 0)                                                              ;; Put a 0 on the stack
    ...
    (if                                                                        ;; Execute code based on result of condition.
      (then                                                                      ;; 'then' block
        ...
        (local.set $var0)                                                          ;; Set var 'val' from stack
      )                                                                          ;; End 'then'
    )                                                                          ;; End 'if'
    (i32.const 27)                                                             ;; Load address of string literal
    (local.set $var4)                                                          ;; Set var 'out' from stack
    (block $exit1                                                              ;; Outer block for breaking while loop.
      (loop $loop1                                                             ;; Inner loop for continuing while.
        ...
        (local.get $var2)                                                          ;; Place var 'digits' onto stack
        (local.get $var0)                                                          ;; Place var 'val' onto stack
        ...
        (local.get $var4)                                                          ;; Place var 'out' onto stack
    ...
    ;; Test condition for if.
    (local.get $var3)                                                          ;; Place var 'is_neg' onto stack
    (if                                                                        ;; Execute code based on result of condition.
      (then                                                                      ;; 'then' block
        (i32.const 2)                                                              ;; Allocate 2 bytes for char string
        call $_alloc_str                                                           ;; Allocate memory for string
        (local.set $temp1)                                                         ;; Store address in local variable
        (local.get $temp1)                                                         ;; Get address of string
        (i32.const 45)                                                             ;; Put a char \45 on the stack
        i32.store8                                                                 ;; Store the char at the address
        (local.get $temp1)                                                         ;; Return address of the string
        (local.get $var4)                                                          ;; Place var 'out' onto stack
        call $_strcat                                                              ;; Concatenate two strings
        (local.set $var4)                                                          ;; Set var 'out' from stack
      )                                                                          ;; End 'then'
    ...
    (local.get $var4)                                                          ;; Place var 'out' onto stack
  )                                                                          ;; END 'Int2String' function definition.
  
  (export "Int2String" (func $Int2String))
