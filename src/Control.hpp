...
struct Control {
  ...
  int indent = 0;
  bool final_node =
      false; // Are we processing the final (right-most) node in a function?
  size_t wat_mem_pos = 14; // Position for generating fixed data in WAT memory.

  std::vector<std::string>
      break_stack; // Stack of break labels for active scopes.
  std::vector<std::string>
      loop_stack; // Stack of continue labels for active scopes.

  // Labels are made unique by adding a number to their end; track of what
  // number we are up to!
  ...
  std::vector<std::pair<std::string, std::string>> temp_vars;
...
public: // Member functions.
  ...
  template <typename... Ts> Control &Code(Ts &&...args) {
    std::stringstream ss;
  ...
  template <typename... Ts> Control &Comment(Ts &&...args) {
    std::stringstream ss;
    (ss << ... << std::forward<Ts>(args));
  ...
  template <typename... Ts> Control &CommentLine(Ts &&...args) {
  ...
  void PrintCode(std::ostream &os = std::cout) const {
    ...
    for (const auto &line : code) {
      ...
      if (line.comment.size()) {
        if (line.code
                .size()) { // If there is code on this line, align comments.
  ...
  void WATDeclareSymbols(std::vector<size_t> var_ids) {
    ...
    for (size_t i : var_ids) {
      Code("(local $var", i, " ", WATType(i), ")")
          .Comment("Variable: ", symbols.GetName(i));
    }
    Code("");
  }

  // Method to declare a new local variable in the current function
  std::string DeclareTempVar(const std::string &type) {
    std::string var_name = "$temp" + std::to_string(temp_var_counter++);
    temp_vars.emplace_back(var_name, type);
    return var_name;
  }

  // Method to output the local variable declarations when starting a function
  void WATDeclareTempVars() {
    CommentLine("Temp Variables");
    for (const auto &var : temp_vars) {
      Code("(local ", var.first, " ", var.second, ")");
    }
    temp_vars.clear(); // Clear after declaring
    Code("");
  }
