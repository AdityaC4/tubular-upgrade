...
class ASTNode {
protected:
  FilePos file_pos; // What file position was this node parsed from in the
                    // original file?
...
public:
  ...
  FilePos GetFirstPos() const override {
    ...
    for (const auto &child : children) {
      if (child->GetFirstPos() < first_pos)
        first_pos = child->GetFirstPos();
    }
    return first_pos;
  }
  ...
  template <typename NODE_T> void AdaptChild(size_t id) {
  ...
  void Print(std::string prefix = "") const override { PrintChildren(prefix); }
...
public:
  ...
  void AddChild(ptr_t &&child) override {
    ...
    if (is_return)
      Error(child->GetFirstPos(), "Unreachable code.");
    // If the child we are adding is a return statement, this block ends in a
    // return.
    if (child->IsReturn())
      is_return = true;
    if (child->MayReturn())
  ...
  bool ToWAT(Control &control) override {
    ...
    for (size_t i = 0; i < NumChildren(); ++i) {
      ...
      if (i == NumChildren() - 1)
        control.FinalNode(is_final_node);
...
public:
  ...
  bool ToWAT(Control &control) override {
    ...
    // First, generate code for the function body to populate temp_vars
    std::vector<Control::WAT_Line> body_code;
    {
      auto old_code = std::move(control.code);
      control.code.clear();

      control.FinalNode(true); // Since there is only one node in this function
      ChildToWAT(0, control, false);

      body_code = std::move(control.code);
      control.code = std::move(old_code);
    }

    // declare variables, including temp variables
    control.WATDeclareSymbols(var_ids);
    control.WATDeclareTempVars();

    // Append the function body code
    control.code.insert(control.code.end(), body_code.begin(), body_code.end());

    control.Indent(-2);
    control.Code(")")
        .Comment("END '", fun_name, "' function definition.")
        .Code("") // Skip a line.
...
public:
  ...
  bool ToWAT(Control &control) override {
    ...
    control.Code("(if ", result_str, "")
        .Comment("Execute code based on result of condition.")
        .Indent(2)
        .Code("(then")
        .Comment("'then' block")
        .Indent(2);
...
public:
  ...
  bool ToWAT(Control &control) override {
    ...
    // A while loop may go around again, so we cannot treat any node inside of
    // it as final. (In practice, though programs functions should end with a
    // while anyway)
    control.FinalNode(false);
    ...
    control.Code("(block ", while_exit, "")
        .Comment("Outer block for breaking while loop.")
        .Code("  (loop ", while_loop, "")
        .Comment("Inner loop for continuing while.");
    control.Indent(4);
    control.CommentLine("WHILE Test condition...");

    ChildToWAT(0, control, true);

    control.Code("(i32.eqz)")
        .Comment("Invert the result of the test condition.")
        .Code("(br_if ", while_exit, ")")
        .Comment("If condition is false (0), exit the loop")
    ...
    control.CommentLine("WHILE start next loop.")
        .Code("(br ", while_loop, ")")
        .Comment("Jump back to the start of the loop");
    control.Indent(-4);
    control.Code("  )").Comment("End loop").Code(")").Comment("End block");
...
public:
  ...
  bool ToWAT(Control &control) override {
    if (!control.HasLoopLabel())
      Error(file_pos, "No loop for `break` to exit.");
...
public:
  ...
  bool ToWAT(Control &control) override {
    if (!control.HasLoopLabel())
      Error(file_pos, "No loop for `continue` to operate on.");
...
public:
  ...
  void TypeCheck(const SymbolTable &symbols) override {
    ...
    TypeCheckChildren(symbols);
    const Type &child_type = GetChild(0).ReturnType(symbols);
    if (!Type("string").CastFromOK(child_type)) {
      Error(file_pos, "Cannot cast type ", child_type.Name(), " to string.");
    }
  }

  bool ToWAT(Control &control) override {
    assert(NumChildren() == 1);
    const Type &child_type = GetChild(0).ReturnType(control.symbols);
    if (child_type.IsChar()) {
      // Generate code to convert char to string
      std::string str_addr = control.DeclareTempVar("i32");
      GenerateCharToString(control, str_addr);
    } else if (child_type.IsInt()) {
...
private:
  void GenerateCharToString(Control &control, const std::string &str_addr) {
    // Allocate memory for a 2-byte string (1 character + null terminator)
    ...
    ChildToWAT(0, control, true); // Generate code for the char expression
  ...
  void GenerateIntToString(Control &control) {
    ChildToWAT(0, control, true); // Generate code for the int expression
    control.Code("call $_int2string").Comment("Convert int to string");
  }
};
...
public:
  ...
  void TypeCheck(const SymbolTable &symbols) override {
    ...
    if (op == "*") {
      if ((type0.IsString() || type0.IsChar()) && type1.IsInt()) {
        if (type0.IsChar()) {
      ...
      if (type0.IsString() && type1.IsString()) {
        status = OK;
      } else if (type0.IsString() && type1.IsChar()) {
        AdaptChild<ASTNode_ToString>(1);
        status = OK;
      } else if (type1.IsString() && type0.IsChar()) {
        AdaptChild<ASTNode_ToString>(0);
        status = OK;
      } else {
        align_numeric = true;
      ...
      if (type0 == type1 && !type0.IsFunction())
        status = OK;

      // Otherwise see about promote RHS only.
      else if (type0.IsDouble() && type1.IsNumeric())
        status = PROMOTE1_DOUBLE;
      else if (type0.IsInt() && type1.IsChar())
        status = PROMOTE1_INT;
      else if (type0.IsString() && (type1.IsString() || type1.IsChar()))
        status = OK;
    } else {
      // Internal error
    ...
    if constexpr (DEBUG) {
      ...
      switch (status) {
      case PROMOTE0_INT:
        std::cerr << "...Promoting 0 to INT" << std::endl;
        break;
      case PROMOTE0_DOUBLE:
        std::cerr << "...Promoting 0 to DOUBLE" << std::endl;
        break;
      case PROMOTE1_INT:
        std::cerr << "...Promoting 1 to INT" << std::endl;
        break;
      case PROMOTE1_DOUBLE:
        std::cerr << "...Promoting 1 to DOUBLE" << std::endl;
        break;
      default:
        break;
      }
    ...
    case OK:
      break;
    case PROMOTE0_INT:
      AdaptChild<ASTNode_ToInt>(0);
      break;
    case PROMOTE0_DOUBLE:
      AdaptChild<ASTNode_ToDouble>(0);
      break;
    case PROMOTE1_INT:
      AdaptChild<ASTNode_ToInt>(1);
      break;
    case PROMOTE1_DOUBLE:
      AdaptChild<ASTNode_ToDouble>(1);
      break;
    }
  }
  ...
  void ToWAT_Assign(Control &control) {
    if (!GetChild(0).CanAssign()) {
      Error(file_pos, "Left-hand-side of assignment must be a assignable.");
    }
    ...
    ChildToWAT(0, control,
               true); // Place the current value of var on the stack.
  }
  ...
  void ToWAT_AND(Control &control) {
    ...
    control.Code("(i32.const 0)")
        .Comment("Put a zero on the stack for comparison)")
        .Code("(i32.ne)")
        .Comment("Set any non-zero value to one.)")
        .Indent(-4)
        .Code("  )")
        .Code("  (else ")
        .Code("    (i32.const 0)")
        .Comment("First clause of && was false.")
        .Code("  )")
        .Code(")")
  ...
  void ToWAT_OR(Control &control) {
    ...
    control.Code("(if (result i32)")
        ...
        .Code("  (then")
        .Code("    (i32.const 1)")
        .Comment("First clause of || was true.")
        .Code("  )")
        .Code("  (else ")
        .Indent(4);
  ...
  void ToWAT_Multiply(Control &control) {
    Type type0 = GetChild(0).ReturnType(control.symbols);
    Type type1 = GetChild(1).ReturnType(control.symbols);
    if (type0.IsNumeric()) {
      // Standard mathematical multiple.
      ChildToWAT(0, control, true);
      ChildToWAT(1, control, true);
      control.Code("(", type0.ToWAT(), ".mul)").Comment("Stack2 * Stack1");
    } else if (type0.IsString() && type1.IsInt()) {
      ChildToWAT(0, control, true);
      ChildToWAT(1, control, true);
      control.Code("call $_repeat_string").Comment("Multiply string");
    } else if (type0.IsChar() && type1.IsInt()) {
      AdaptChild<ASTNode_ToString>(0);
      ChildToWAT(0, control, true);
      ChildToWAT(1, control, true);
      control.Code("call $_repeat_string").Comment("Multiply string");
  ...
  void ToWAT_Add(Control &control) {
    const Type &type0 = GetChild(0).ReturnType(control.symbols);
    const Type &type1 = GetChild(1).ReturnType(control.symbols);

    if (type0.IsNumeric() && type1.IsNumeric()) {
      // Standard mathematical addition.
      control.Code("(", type0.ToWAT(), ".add)").Comment("Stack2 + Stack1");
    } else if (type0.IsString() && type1.IsString()) {
      control.Code("call $_strcat").Comment("Concatenate two strings");
    }
  }
  ...
  bool ToWAT(Control &control) override {
    ...
    // If we are doing an assignment or boolean logic, we need to handle it
    // specially.
    if (op == "=") {
      ToWAT_Assign(control);
      return true;
    }
    if (op == "&&") {
      ToWAT_AND(control);
    ...
    if (op == "/") {
      control.Code("(", type, ".div", extra, ")").Comment("Stack2 / Stack1");
      return true;
    }
    if (op == "%") {
      control.Code("(", type, ".rem", extra, ")").Comment("Stack2 % Stack1");
      return true;
    }
    if (op == "+") {
      ToWAT_Add(control);
    ...
    if (op == "-") {
      control.Code("(", type, ".sub)").Comment("Stack2 - Stack1");
      return true;
    }

    if (op == "<") {
      control.Code("(", type, ".lt", extra, ")").Comment("Stack2 < Stack1");
      return true;
    }
    if (op == "<=") {
      control.Code("(", type, ".le", extra, ")").Comment("Stack2 <= Stack1");
      return true;
    }
    if (op == ">") {
      control.Code("(", type, ".gt", extra, ")").Comment("Stack2 > Stack1");
      return true;
    }
    if (op == ">=") {
      control.Code("(", type, ".ge", extra, ")").Comment("Stack2 >= Stack1");
      return true;
    }
    if (op == "==") {
      if (GetChild(0).ReturnType(control.symbols).IsString()) {
        control.Code("call $_str_cmp").Comment("call string compare function");
      } else {
        control.Code("(", type, ".eq)").Comment("Stack2 == Stack1");
      }
      return true;
    }
    if (op == "!=") {
      control.Code("(", type, ".ne)").Comment("Stack2 != Stack1");
      return true;
    }
...
public:
  ASTNode_Indexing(FilePos file_pos, ptr_t &&base_expr, ptr_t &&index_expr)
      : ASTNode_Parent(file_pos, base_expr, index_expr) {}

  std::string GetTypeName() const override { return "indexing"; }

  Type ReturnType(const SymbolTable &symbols) const override {
    const Type &base_type = GetChild(0).ReturnType(symbols);
  ...
  bool ToWAT(Control &control) override {
    ChildToWAT(0, control, true);
    ChildToWAT(1, control, true);
    control.Code("(i32.add)").Comment("Computer address: base + index");
...
public:
  ASTNode_Size(FilePos file_pos, ptr_t &&arg) : ASTNode_Parent(file_pos, arg) {}
  std::string GetTypeName() const override { return "SIZE"; }

  Type ReturnType(const SymbolTable &) const override { return Type("int"); }

  void TypeCheck(const SymbolTable &symbols) override {
    if (NumChildren() != 1) {
      Error(file_pos, "size() expects one argument.");
    }
    TypeCheckChildren(symbols);
