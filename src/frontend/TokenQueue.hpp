...
//   bool found = tokens.UseIf('$'); // Use the next token IF it is a dollar
//   sign auto token2 = tokens.Peek();    // Get the next token _without_
//   advancing
//
//   // Use the next token; if it is NOT an IDENTIFIER token, give an error and
//   exit! auto token3 = tokens.Use(Lexer::ID_IDENTIFIER);
//
...
public:
  ...
  template <typename... Ts> const emplex::Token &Use(int id, Ts &&...message) {
    if (!Is(id)) {
      if constexpr (sizeof...(Ts) == 0) {
        Error(CurFilePos(), "Expected token of type ",
              emplex::Lexer::TokenName(id), ", but found type ",
              emplex::Lexer::TokenName(Peek()));
      } else
        Error(CurFilePos(), std::forward<Ts>(message)...);
    }
    return Use();
  }
  ...
  template <typename... Ts> int UseIf(int id, Ts... args) {
    if (Is(id)) {
      return Use();
    }
    return UseIf(args...);
  ...
  FilePos CurFilePos() const {
    if (Any())
      return FilePos{Peek().line_id, Peek().col_id};
    else
      return FilePos{0, 0};
  }
};
